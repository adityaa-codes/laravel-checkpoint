...
final readonly class BuildReplicationCommandPayloadAction
{
    ...
    {
        if ($input === '') {
            throw new InvalidArgumentException(sprintf('Replication %s endpoint is required.', $role));
        }
    ...
    private function resolveCriticalTables(array $tables): array
    {
        ...
        if (! is_array($configTables)) {
            throw new InvalidArgumentException('checkpoint.replication.critical_tables must be an array of non-empty strings.');
        }
    ...
    private function normalizeCriticalTables(array $tables): array
    {
        ...
        foreach ($tables as $table) {
            if (! is_string($table)) {
                throw new InvalidArgumentException('Critical tables must be non-empty strings.');
            }
